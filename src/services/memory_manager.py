...
class ConversationMemoryManager:
    ...
    def add_user_message(self, message: str) -> None:
        ...
        self.turn_number += 1
        logger.debug(
            "User message added", turn=self.turn_number, message_length=len(message)
        )
    ...
    def add_ai_message(self, message: str) -> None:
        ...
        logger.debug(
            "AI message added", turn=self.turn_number, message_length=len(message)
        )
    ...
    def get_conversation_history(self) -> List[Dict[str, Any]]:
        ...
        for i, msg in enumerate(messages):
            if isinstance(msg, HumanMessage):
                history.append(
                    {"role": "user", "content": msg.content, "turn": (i // 2) + 1}
                )
            elif isinstance(msg, AIMessage):
                history.append(
                    {"role": "assistant", "content": msg.content, "turn": (i // 2) + 1}
                )
