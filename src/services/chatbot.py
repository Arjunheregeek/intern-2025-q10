...
class CachedChatbot:
    ...
    async def display_db_stats(self) -> None:
        ...
        try:
            ...
            print(
                f"  • Average response time: {stats['average_response_time_ms']:.1f} ms"
            )
        ...
        try:
            ...
            with open(filename, "w", encoding="utf-8") as f:
                ...
                for i, chat in enumerate(all_history, 1):
                    ...
                    f.write(
                        f"Metrics: {chat['tokens_used']} tokens, {chat['response_time_ms']:.1f}ms\n"
                    )
                    f.write("\n")
    ...
    def generate_response(self, user_input: str) -> Optional[str]:
        ...
        try:
            ...
            # Store in database
            asyncio.create_task(
                self._store_in_database(user_input, response, api_latency)
            )
    ...
    async def _store_in_database(
        self, prompt: str, response: str, latency_ms: float
    ) -> None:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            print(
                f"  • Time saved by caching: {stats['total_time_saved_ms']/1000:.1f}s"
            )
    ...
    async def _run_async(self) -> None:
        ...
        while self.is_running:
            try:
                user_input = input(
                    f"\n[Cache: {self.cache.get_stats()['cache_size']}/50] You: "
                ).strip()
