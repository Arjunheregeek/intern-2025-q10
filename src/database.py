...
class DatabaseManager:
    ...
    async def create_schema(self, db: aiosqlite.Connection):
        ...
        try:
            await db.execute("ALTER TABLE chat_history ADD COLUMN created_at DATETIME DEFAULT CURRENT_TIMESTAMP;")
    ...
    async def create_indexes(self, db: aiosqlite.Connection):
        ...
        if 'prompt_hash' in column_names:
            indexes.append("CREATE INDEX IF NOT EXISTS idx_prompt_hash ON chat_history(prompt_hash);")
            
        if 'created_at' in column_names:
            indexes.append("CREATE INDEX IF NOT EXISTS idx_created_at ON chat_history(created_at DESC);")
    ...
    async def store_chat_interaction(
        ...
        try:
            ...
            async with self.get_connection() as db:
                ...
                logger.info(f"Stored chat interaction: ID={interaction_id}, Session={session_id}")
    ...
    async def get_usage_stats(self) -> Dict[str, Any]:
        ...
        try:
            async with self.get_connection() as db:
                ...
                # Unique sessions
                cursor = await db.execute("SELECT COUNT(DISTINCT session_id) as unique_sessions FROM chat_history")
                sessions_row = await cursor.fetchone()
                unique_sessions = sessions_row['unique_sessions'] if sessions_row else 0
                
                # Average response time
                cursor = await db.execute("SELECT AVG(response_time_ms) as avg_response_time FROM chat_history WHERE response_time_ms > 0")
                avg_time_row = await cursor.fetchone()
                avg_response_time = avg_time_row['avg_response_time'] if avg_time_row else 0
                
                # Total tokens used
                cursor = await db.execute("SELECT SUM(tokens_used) as total_tokens FROM chat_history")
