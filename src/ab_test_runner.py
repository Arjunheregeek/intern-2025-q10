...
class ABTestRunner:
    ...
    def simulate_llm_call(self, system_prompt: str, user_query: str) -> tuple[str, float]:
    ...
    def run_evaluation(self) -> None:
        ...
        for _, row in queries_df.iterrows():
            ...
            for version in ['A', 'B']:
                ...
                score = self._simulate_quality_score(query, version, row['favors_prompt'])
    ...
    def save_results(self) -> None:
        ...
        with open(results_path, 'w', newline='', encoding='utf-8') as file:
            ...
            writer.writerow([
                'query', 'prompt_version', 'response', 'score', 'latency_ms',
                'timestamp', 'category', 'expected_difficulty', 'favors_prompt'
            ])
            
            for result in self.results:
                writer.writerow([
                    result.query, result.prompt_version, result.response,
                    result.score, result.latency_ms, result.timestamp.isoformat(),
                    result.category, result.expected_difficulty, result.favors_prompt
                ])
